...
export default function AdminPage() {
  ...
  const [approvalFormData, setApprovalFormData] = useState({ treeId: '', role: 'editor' })
  ...
  const { data: pendingUsers = [], isLoading: pendingLoading, refetch: refetchPending } = useQuery({
    ...
    queryFn: () => api.get('/auth/pending-users').then(r => r.data),
  ...
  const handleApproveUser = async (userId) => {
    if (!approvalFormData.treeId) {
      alert('Vui lòng chọn cây gia phả')
      return
    }
    try {
      await api.put(`/auth/approve-user/${userId}/${approvalFormData.treeId}/${approvalFormData.role}`)
      toast.success('Phê duyệt người dùng thành công')
      refetchPending()
      setSelectedUserForApproval(null)
      setApprovalFormData({ treeId: '', role: 'editor' })
    } catch (err) {
  ...
  const handleRejectUser = async (userId) => {
    if (!window.confirm('Xác nhận từ chối người dùng này?')) return
    try {
      await api.put(`/auth/reject-user/${userId}`)
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex gap-1 border-b border-amber-200">
        ...
        >
          Quản lý người dùng ({treeUsers.length})
        </button>
        <button
          ...
          style={{fontFamily: 'Georgia, serif'}}
        >
          Phê duyệt đăng ký ({pendingUsers.length})
      ...
      {activeTab === 'pending' && (
        <div className="space-y-6">
          {pendingLoading ? (
            ...
            <div className="text-center py-16 text-amber-700 font-light" style={{fontFamily: 'Georgia, serif'}}>
              Không có yêu cầu đăng ký đang chờ xét duyệt
            </div>
          ) : (
            <div className="space-y-4">
              {pendingUsers.map(user => (
                <div key={user.id} className="bg-amber-50 border-2 border-amber-200 p-4 rounded-sm">
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <h3 className="font-medium text-amber-950" style={{fontFamily: 'Georgia, serif'}}>{user.username}</h3>
                      <p className="text-xs text-amber-600 mt-1">Đăng ký lúc: {new Date(user.createdAt).toLocaleDateString('vi-VN', { 
                        year: 'numeric', 
                        month: '2-digit', 
                        day: '2-digit', 
                        hour: '2-digit', 
                    ...
                    <div className="flex gap-2">
                      {selectedUserForApproval === user.id ? (
                        <button
                          onClick={() => setSelectedUserForApproval(null)}
                          className="px-3 py-1 text-gray-600 hover:bg-gray-200 transition-colors text-sm border border-gray-300 rounded-sm"
                          style={{fontFamily: 'Georgia, serif'}}
                        >
                          Đóng
                        </button>
                        ...
                        <>
                          <button
                            ...
                            className="px-3 py-1 bg-green-600 text-white hover:bg-green-700 transition-colors text-sm border border-green-700 rounded-sm"
                            style={{fontFamily: 'Georgia, serif'}}
                          >
                            Phê duyệt
                          </button>
                          <button
                            ...
                            className="px-3 py-1 bg-red-600 text-white hover:bg-red-700 transition-colors text-sm border border-red-700 rounded-sm"
                            style={{fontFamily: 'Georgia, serif'}}
                          >
                            Từ chối
                          </button>
                  ...
                  {selectedUserForApproval === user.id && (
                    <div className="bg-white p-4 rounded-sm border border-amber-200 mt-4 space-y-3">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div>
                          <label className="block text-sm font-light text-amber-900 mb-1" style={{fontFamily: 'Georgia, serif'}}>Cây gia phả *</label>
                          <select
                            value={approvalFormData.treeId}
                            onChange={(e) => setApprovalFormData({...approvalFormData, treeId: e.target.value})}
                            className="w-full px-3 py-2 border border-amber-200 rounded-sm text-sm focus:outline-none focus:border-amber-900 bg-white"
                            style={{fontFamily: 'Georgia, serif'}}
                          >
                            <option value="">-- Chọn cây gia phả --</option>
                            <option value={treeId}>{currentTree?.name}</option>
                          </select>
                        </div>
                      ...
                      <div className="flex gap-2 pt-2">
                        <button
                          ...
                          className="px-4 py-2 bg-green-600 text-white text-sm font-light hover:bg-green-700 transition-colors rounded-sm"
                          style={{fontFamily: 'Georgia, serif'}}
                        >
                          Xác nhận phê duyệt
                        </button>
      ...
      {/* Info Box - only show in users tab */}
      {activeTab === 'users' && (
